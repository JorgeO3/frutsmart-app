...
# IMPORTA recursos ya existentes para que Terraform los gestione
import {
  to = azurerm_log_analytics_workspace.law
  id = "/subscriptions/df817eb4-4138-41f6-a68d-42e0d2354bbe/resourceGroups/aidata-rg/providers/Microsoft.OperationalInsights/workspaces/aidata-law"
}

import {
  to = azurerm_virtual_network.vnet
  id = "/subscriptions/df817eb4-4138-41f6-a68d-42e0d2354bbe/resourceGroups/aidata-rg/providers/Microsoft.Network/virtualNetworks/aidata-vnet"
}
...
resource "azurerm_consumption_budget_subscription" "monthly_budget" {
  ...
  notification {
    ...
    threshold      = 50
    contact_emails = ["finops@empresa.com"]
  }
  notification {
    enabled        = true
    operator       = "GreaterThanOrEqualTo"
    threshold      = 75
    contact_emails = ["ops@empresa.com"]
  }
  notification {
    enabled        = true
    operator       = "GreaterThanOrEqualTo"
    threshold      = 100
    contact_emails = ["ciso@empresa.com"]
  }
  notification {
    enabled        = true
    ...
    contact_emails = ["finops@empresa.com", "ops@empresa.com"]
  }
}
