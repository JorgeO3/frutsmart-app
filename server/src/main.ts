...
class AppBootstrap {
  ...
  private getCorsConfig(): Record<string, unknown> {
    ...
    const allowedOrigins = this.parseOrigins(process.env.BACKEND_CORS_ORIGINS);
  ...
  private setupGlobalFeatures(): void {
    ...
    this.app.useGlobalPipes(
      new ValidationPipe({
        ...
        enableDebugMessages: process.env.BACKEND_NODE_ENV !== "production",
        validationError: {
          target: false,
          value: process.env.BACKEND_NODE_ENV !== "production",
        },
      }),
    );
  }
