...
declare class SkyboltModule extends NativeModule<SkyboltModuleEvents> {
  // Configuration
...
try {
  skyboltModule = requireNativeModule<SkyboltModule>('Skybolt');
} catch {
  skyboltModule = null;
}
 ...
 */
function assertModule(): SkyboltModule {
  if (!skyboltModule) {
    throw new Error(
      '[Skybolt] Native module not found. Did you build the app?'
    );
  }
  return skyboltModule;
}
...
export function addUploadListener(
  ...
  return module.addListener('onUploadEvent', (native) => {
    try {
...
export async function configure(settings: CloudUploadSettings): Promise<void> {
  ...
  return assertModule().configure(configWithDefaults);
}
...
export async function initializeSession(config: SessionConfig): Promise<void> {
  return assertModule().initializeSession(config);
}
...
export async function setAuthTokens(tokens: AuthTokens): Promise<void> {
  return assertModule().setAuthTokens(tokens);
}
