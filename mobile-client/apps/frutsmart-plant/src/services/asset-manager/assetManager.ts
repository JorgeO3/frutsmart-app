...
const MODELS_DIR = `${DOCS_DIR}models/`;
const REPORTS_ASSETS_DIR = `${DOCS_DIR}reports/assets/`;

// biome-ignore lint/suspicious/noExplicitAny: Allow any type for AssetMap
...
class InitialAssetsManager {
  ...
  public async setup(): Promise<void> {
    ...
    try {
      // const [dbMap, modelsMap, reportsMap] = await Promise.all([
      //   import('./asset-maps/db.map'),
      //   import('./asset-maps/models.map'),
      //   import('./asset-maps/reports.map'),
      // ]);

      const [dbMap] = await Promise.all([
        import('./asset-maps/db.map'),
      ]);
      ...
      await Promise.all([
        ...
        // this._setupAssetGroup(modelsMap.default, MODELS_DIR, 'Modelos de IA'),
