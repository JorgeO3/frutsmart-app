...
// --- Constantes ---
const DATABASE_ASSET_PATH = require("@/assets/db/frutsmart_plant_database.db");
const DATABASE_SCHEMA_PATH = require("@/assets/db/frutsmart_plant_schema.sql");
  ...
  private async initializeDatabase(): Promise<SQLiteDatabase> {
    try {
      ...
      if (ENABLE_MOCK_DATA && !dbFileInfo.exists) {
        ...
        if (!dbAsset.localUri) {
          throw new Error(
            "No se pudo obtener la URI local para el asset de la base de datos.",
          );
        }
      ...
      // (CREATE TABLE IF NOT EXISTS es idempotente, no afecta datos previos).
      // Esto permite que nuevas tablas añadidas al schema se creen en DBs existentes.
      console.log("Ejecutando esquema SQL para sincronizar tablas...");
