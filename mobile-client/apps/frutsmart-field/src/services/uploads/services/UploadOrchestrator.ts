...
class UploadOrchestrator {
  ...
  async createJob(analysisId: string, domain: "plant" | "field" = "field"): Promise<string> {
    ...
    const row: UploadJobRow = {
      id: jobId,
      quality_classification_id: analysisId,
      domain,
  ...
  private rowToMachine(row: UploadJobRow): { ctx: UploadJobContext; state: UploadStateValue } {
    const ctx: UploadJobContext = {
      jobId: row.id,
      analysisId: row.quality_classification_id ?? "",
      domain: row.domain,
  ...
  private async buildAnalysisFiles(analysisId: string): Promise<AnalysisUploadFile[]> {
    const classification = await database.qualityClassifications.findById(analysisId);
    if (!classification) {
      throw new Error(`No se encontró quality_classification_id=${analysisId}`);
    }
