...
import SessionDetailsModal from "@components/app/field-work/uploads/SessionDetailsModal";
...
const mapStatus = (
  ...
  if (job.pipelineStep === "create_session") {
    return {
      status: "preparing",
      label: "Preparando clasificacion",
  ...
  if (job.pipelineStep === "upload" && job.status === "running") {
    return {
      ...
      message: "Subiendo evidencias de la clasificacion.",
    };
  }
...
const mapJobToSession = (
  ...
  return {
    id: job.id,
    name: toClassificationLabel(job.qualityAnalysisId, job.id),
