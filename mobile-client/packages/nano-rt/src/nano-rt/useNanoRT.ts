...
import { isAvailable, NativeNanoRT } from './NanoRTModule';
...
type Subscription = { remove: () => void } | (() => void) | null | undefined;

const createUnsubscriber = (subscription: Subscription): (() => void) | undefined => {
  if (typeof subscription === 'function') return subscription;
  if (subscription && typeof subscription === 'object' && 'remove' in subscription && typeof subscription.remove === 'function') {
...
export function useNanoRT(options: UseNanoRTOptions = {}): ReadyState & {
  ...
  const [state, setState] = useState<ReadyState>({
    initializing: autoInitialize && isAvailable,
    ready: false,
    error: isAvailable
      ? null
      : {
          message: '[NanoRT] Native module not found. Did you build the app?',
