...
import NativeSkybolt from '../../specs/NativeSkybolt';
import type {
  KeyValuePair,
  NativeCloudUploadSettings,
  NativeSessionConfig,
  NativeUploadEvent as NativeUploadEventSpec,
} from '../../specs/NativeSkybolt';
import type {
  AuthTokens,
  CloudUploadSettings,
  ItemProgress,
...
type SkyboltApi = {
  ...
  onUploadEvent(listener: (event: NativeUploadEventSpec) => void): EventSubscription | LegacySubscription;
};
...
export function addUploadListener(
  ...
  return module.onUploadEvent(nativeEvent => {
